...
public abstract class TransportClientUtil {
	public static TransportClient buildTransportClient(Environment env){
		...
		for(String urlString : env.getRequiredProperty("dilicom.sync.elasticsearch.urls").split(";")){
			String[] url = urlString.split(":");
			
			if(url.length != 2){
				throw new RuntimeException("Property dilicom.sync.elasticsearch.urls is not properly defined.");
			}
